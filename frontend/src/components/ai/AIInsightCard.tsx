      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-xl tracking-tight">Opportify Insight</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[40px] font-black leading-none">{insight.score}%</span>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
             <div className="space-y-3">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-300">Điểm nổi bật</h4>
                <div className="flex flex-wrap gap-2">
                  {insight.pros?.map((pro: string, i: number) => (
                    <span key={i} className="px-3 py-1 bg-emerald-500/20 text-emerald-300 rounded-lg text-xs font-bold flex items-center gap-1.5 border border-emerald-500/30">
                      <Target size={12} /> {pro}
                    </span>
                  ))}
                </div>
             ...
             <div className="space-y-3 pt-2">
                <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-300">Lời khuyên AI</h4>
                <div className="flex flex-col gap-2">
                  {insight.tips?.map((tip: string, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-xs text-indigo-100 font-medium bg-white/5 p-3 rounded-xl">
